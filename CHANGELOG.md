...
* Add support for more operators in `Style/ConditionalAssignment`. ([@rrosenblum][])
...
* Fix auto-correcting `===` in `Style/ConditionalAssignment`. ([@rrosenblum][])
...
* `Style/ConditionalAssignment` now accounts for `+` and `-`. ([@rrosenblum][])
